...
const DeleteUser = ({ open, setopen,  payload,  onClose,SetUsers,users }) => {
    ...
    const handleDelete = async () => {
        try {
            setIsDeleting(true);
            await Axios.delete(`http://localhost:1500/api/user/${payload._id}`);
            SetUsers(users.filter(item => item._id !== payload._id));
            setopen(false)
            ...
            <DialogContent>
                {error && (
                    <Alert severity="error">
                        {`An error occurred while deleting item "${payload.name}":`}
                        <pre>{error}</pre>
                    </Alert>
                )}
            </DialogContent>
